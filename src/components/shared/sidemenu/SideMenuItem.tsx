...
export const SideMenuItem = ({ href, Icon, title, subTitle, expanded = false }: Props) => {
        ...
        <div className="flex flex-row justify-between">
          ...
          {expanded && <div className="pl-5">
            ...
            <div className="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">
              {hasOpen && <ul id="dropdown-example" className=" py-2 space-y-2">
                {hasOpen && units.map((item) => (<li
                  className="flex items-center w-full p-2 text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" key={item.id}>{item.name}</li>))}
              </ul>}
            </div>
          </div>}
        </div>
